                ...
                <HoverCardContent className="w-80" align="end">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        id="country"
                        value={usuario?.id_pais ? String(usuario.id_pais) : userProfile.preferredCountry}
                        onChange={async (e) => {
                          const id_pais = Number(e.target.value);
                          setUserProfile({ ...userProfile, preferredCountry: e.target.value });
                          if (!usuario) return;
                          try {
                            await usuariosService.update(usuario.id, { id_pais });
                            setUsuario((prev) => prev && { ...prev, id_pais });
                          } catch (err) {
                      ...
                      >
                        ...
                        {!paisesLoading && !paisesError && backendPaises?.map((country) => (
                          <option key={country.id} value={String(country.id)}>
                            {/* bandera comentada para implementar después: {country.flag}  */}
                            {country.nombre}
                          </option>
                        ))}
                      </select>
